...
# =====================================================
# AUTO-CLEAR ChromaDB on first load of a new session
# Ensures Documents in DB always starts at 0 when the
# user opens the app fresh (new browser tab / server
# restart).  The flag db_initialized lives only in
# session_state so it resets whenever the session ends.
# =====================================================
if not st.session_state.get("db_initialized"):
    try:
        vs = get_vectorstore(collection_name)
        existing_ids = vs._collection.get().get("ids", [])
        if existing_ids:
            vs._collection.delete(existing_ids)
    except Exception:
        pass  # Silently ignore if chroma_db doesn't exist yet
